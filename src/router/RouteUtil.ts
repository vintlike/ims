import type { AsyncRouteType, MenuItem, RouteItem } from '@/router/route';
import store from '@/store';
import { setStoreAsyncRouter } from '@/store/modules/routeReducer';
import { cloneDeep } from 'lodash';
import type { Key } from 'react';
import { defaultRouter } from '.';
import { getRouteApi } from '@/services/route';
...
export function handlePowerRoute(dataRouter: AsyncRouteType[], routerList: RouteItem[] = defaultRouter) {
...
export function getParentPaths(routePath: string, routes: MenuItem[]): string[] {
  ...
  function dfs(routes: MenuItem[], key: string, parents: string[]) {
    for (let i = 0; i < routes.length; i++) {
      ...
      if (dfs(item.children, key, parents).length) {
        return parents;
      }
      // 深度遍历查找未找到时当前path 出栈
      parents.pop();
